...
function ChatArea({ socket }) {
  ...
  useEffect(() => {
    ...
    // clear unread messages from server using the sockets
  ...
  const onUploadImageClick = async (e) => {
    ...
    reader.onloadend = async () => {
      sendNewMessage(reader.result, "Image");
    };
  };
  ...
  return (
    <div className="bg-white h-[82vh] border rounded-2xl w-full flex flex-col justify-between p-5">
      ...
      {/* 2nd part include chat messages  */}
      <div className="h-[60vh] overflow-y-scroll p-5" id="messages">
        <div className="flex flex-col gap-2">
          {messages.map((message, index) => {
            ...
            return (
              <div className={`flex ${isCurrentUserIsSender && "justify-end"}`}>
                ...
                {/* // Blue and Grey tick for Message Seen or Unseen */}
                {/* <i
          ...
          {isReceipentTyping && (
            <div className="pb-10">
              <h1 className="bg-blue-100 text-white p-2 rounded-xl w-max">
                typing...
              </h1>
            </div>
          )}
      ...
      {/* 3 rd part will be chat input */}

      <div className="h-18 rounded-xl border-gray-300 shadow borrder flex justify-between p-2 items-center relative">
        {showEmojiPicker && (
